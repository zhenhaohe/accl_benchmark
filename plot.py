...
def read_host_device_log_files(log_dir):
    ...
    column_names = [
        "count", "number_of_nodes", "fpga_to_host", 
        "host_to_fpga", "size"
    ]
    ...
    grouped_df = mpi_dataframes_con.groupby(["count", "number_of_nodes"]).mean()
        ...
        if collective != 'sendrecv':
            ...
            for host_idx, (host, host_group) in enumerate(accl_filter_df.groupby('host')):    
                ...
                if host == "device":
                    ...
                    mpi_avg_time = (mpi_filter_df["host_to_host"] + hd_filter_df['fpga_to_host'] + hd_filter_df['host_to_fpga']).reset_index(drop=True)
                    # print(mpi_avg_time)
                    number_of_nodes_values = hd_filter_df.reset_index()['number_of_nodes']
                    # print(number_of_nodes_values)
                    plt.plot(number_of_nodes_values, mpi_avg_time,
                            label=f'mpi-rdma-device',
                            # linestyle='--',
...
if __name__ == "__main__":
    accl_log_dir = "../accl_results"  # Update this to the directory containing your log files
    output_dir = "../plots/"
    ...
    generate_time_nodes_plots(accl_dataframes, mpi_dataframes, host_device_dataframes, output_dir, 256)
